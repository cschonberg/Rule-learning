...
xyplot(singleprop~Time|as.factor(Subject), data=exp1, type=c("p","r","g"), col="blue",col.line="black",xlab="Time",ylab="singleton prop DT")

null.model<-lme(singleprop~1, random=~1|Subject,data=exp1,na.action=na.omit,control=list(opt="optim"))
VarCorr(null.model)
#ICC = 0.0003075848/(0.0003075848+0.0991760712) = .00309

#check if linear time is significant (it's not)
linear.time<-lme(singleprop~Time,random=~1|Subject,data=exp1,na.action=na.omit,control=list(opt="optim"))
summary(linear.time)$tTable

#check if quadratic time is significant (it's not)
quad.time<-lme(singleprop~poly(Time,2),random=~1|Subject,data=exp1,na.action=na.omit,control=list(opt="optim"))
summary(quad.time)$tTable

#check if cubic time is significant (it's not)
cube.time<-update(quad.time,singleprop~poly(Time,3))
...
tapply(exp1$singleprop,exp1$Time,var,na.rm=T)
